...
centers(1,1,:) = [-19       79.7       47.15] ;
centers(1,2,:) = [-28       77      46.95] ;
centers(1,3,:) = [-36       73      47.85] ;
centers(1,4,:) = [-44       68      48] ;
centers(1,5,:) = [-51.2     61.6        47.2] ;
% % % 
centers(2,1,:) = [-20     83.6        37.75] ;
centers(2,2,:) = [-30       80      39] ;
centers(2,3,:) = [-39       76.3        39.4]  ;
centers(2,4,:) = [-47       70.45        38.45] ;
centers(2,5,:) = [-53.7     63      38.75] ;
% % % 
% % 
centers(3,1,:) = [-23       87.6      28] ;
centers(3,2,:) = [-32.5     83.6        29] ;
centers(3,3,:) = [-41.2     78.85        29.4] ;       % P3 center : [48.1      71.9        29] ;
centers(3,4,:) = [-48.9       72.8      29.3] ;
centers(3,5,:) = [-56       64.8      29] ;
% %
centers(4,1,:) = [-24.6     90.7        19.65] ;
centers(4,2,:) = [-33.8     86.15        18.95] ;
centers(4,3,:) = [-43.25       80.1      19.4] ;
centers(4,4,:) = [-50.4     73.55        18.95] ;
centers(4,5,:) = [-56.8     65.8      19.6] ;
% % 
% % % 
centers(5,1,:) = [-26.2     92.2        10] ;
centers(5,2,:) = [-35.5     87.6        9.4] ;
centers(5,3,:) = [-44.2       81      9.3] ;
centers(5,4,:) = [-51.6     74.6        9.5] ;
centers(5,5,:) = [-58       67.15      9.2] ;
...
T1 = makehgtform('yrotate',pi/2);
T2 = makehgtform('zrotate',0.5*pi/2);
T3 = makehgtform('xrotate',-0.4*pi/2);
%figure; plotmesh(en,ef,'facecolor','r')
%xlabel('X axis'); ylabel('y axis'); zlabel('z axis');
% P3:   T = makehgtform('zrotate',0.2*pi/2,'yrotate',-pi/2);
% T3:   T = makehgtform('yrotate',-pi/2);
en = en * T1(1:3,1:3) * T2(1:3,1:3) * T3(1:3,1:3) ;

% TEST
figure; plotmesh(en,ef,'facecolor','b')
xlabel('X axis'); ylabel('y axis'); zlabel('z axis');
...
% figure; hold on;
% plotmesh(n_all,f_s,'facecolor','b');
...
tic; mesh2comsol_v2(node, face, elem, surface_domains, 'P4grid.mphtxt'); toc_m2c = toc;
