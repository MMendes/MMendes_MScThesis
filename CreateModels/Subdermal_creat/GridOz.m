...
centers(1,1,:) = [17        92.4        16.8] ;
centers(1,2,:) = [7.8      93.1        16.5] ;
centers(1,3,:) = [-4.2     94.02       17] ;
centers(1,4,:) = [-14      94         17] ;
centers(1,5,:) = [-24       92          16.5] ;

centers(2,1,:) = [17       93.6        6.53] ;
centers(2,2,:) = [7        94.1        7] ;
centers(2,3,:) = [-4.2     94.9        6.5] ;
centers(2,4,:) = [-15.4    95.25       6.9] ;
centers(2,5,:) = [-25.2    92.3        7.5] ;


centers(3,1,:) = [16.72     94.02       -3.5] ;
centers(3,2,:) = [6.4       94.5        -3.5] ;
centers(3,3,:) = [-4.27     95.1       -3.49] ;
centers(3,4,:) = [-15.2     94.5        -3.55] ;
centers(3,5,:) = [-25       92.5        -3.3] ;

centers(4,1,:) = [16.8        93.8        -13.7] ;
centers(4,2,:) = [7.2       94.2        -13.8] ;
centers(4,3,:) = [-4.35     94.9        -13.5] ;
centers(4,4,:) = [-15.4     95.08       -12.8] ;
centers(4,5,:) = [-25.7     93          -12.7] ;

centers(5,1,:) = [17.5        88      -23.9] ;
centers(5,2,:) = [7.9       89.4        -22.7] ;
centers(5,3,:) = [-4.2      89.2        -23.4] ;
centers(5,4,:) = [-15.8     88.6        -23.8] ;
centers(5,5,:) = [-25       88      -23.2] ;
...
T1 = makehgtform('yrotate',-0.9*pi/2);
T2 = makehgtform('zrotate',-pi/2);
T3 = makehgtform('xrotate',0.2*pi/2);   % 5th row

en1 = en * T1(1:3,1:3) * T2(1:3,1:3) ;
en2 = en * T1(1:3,1:3) * T2(1:3,1:3) * T3(1:3,1:3) ;
...
for i=1:5
    ...
    for j=1:5
        ...
        Grid.(sprintf('e%d',5*(i-1)+j)).n = en3;  % final ns
        Grid.(sprintf('e%d',5*(i-1)+j)).f = ef;  % final fs

        if i==1 && j==3
            plotmesh(en3,ef,'facecolor','r')
        elseif i==3 && j==3
            plotmesh(en3,ef,'facecolor','g')
        else
        plotmesh(en3,ef,'facecolor','b')
        end
    end 
end
...
%[n_grid,f_grid] = mergemesh( e6_n,e6_f, e7_n,e7_f, e8_n,e8_f ,  e9_n,e9_f, e10_n,e10_f   );
...
tic; mesh2comsol_v2(node, face, elem, surface_domains, 'Ozgrid.mphtxt'); toc_m2c = toc;
