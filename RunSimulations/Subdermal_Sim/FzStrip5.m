%% BRAINCARE
%  Miguel Mendes
%  Last update: 2014.06.01 at 15:00
% ------------------------------------------------------------------------
%       Head Model
%       Subdermal Fz Strip 5
...
model.modelPath('/home/rodrigum/BrainCare/Subdermal/Fz_grid');
...
HM_imp.set('filename', '/home/rodrigum/BrainCare/Subdermal/Fz_grid/Fzgrid.mphtxt');
HM_imp.importData;
...
% 2D boundaries
bound = zeros(5,5);
bound(1,:) = [ 6 19 33 45 47];
bound(2,:) = [ 9 21 29 41 49];
bound(3,:) = [15 25 27 37 55];
bound(4,:) = [13 17 31 39 54];
bound(5,:) = [11 23 35 43 51];
...
%   CSF
csf = model.material.create('maFz');
...
ground.selection.set([27]); % The ID of the half-spherical boundary Fz
...
bound2 = bound(:);
bound2 = bound2(find(bound2~=27)); % remove ground
...
sol1.feature.create('S5', 'Stationary');
sol1.feature('S5').feature.create('fc1', 'FullyCoupled');
sol1.feature('S5').feature.create('i1', 'Iterative');
sol1.feature('S5').feature('i1').set('linsolver', 'cg');
sol1.feature('S5').feature('fc1').set('linsolver', 'i1');
sol1.feature('S5').feature('i1').feature.create('mg1', 'Multigrid');
sol1.feature('S5').feature('i1').feature('mg1').set('prefun', 'amg');
sol1.feature('S5').feature.remove('fcDef');
%sol1.feature('S5').set('nonlin','on');
sol1.attach('std1');
sol1.runAll;
